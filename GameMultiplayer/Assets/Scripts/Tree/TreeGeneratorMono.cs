...
public class TreeGeneratorMono : MonoBehaviour
{
    public static TreeGeneratorMono instance;
    ...
    private void Awake()
    {
        ...
        {
            instance = this;
            if (!trunkMaterial) trunkMaterial = Resources.Load<Material>("Materials/Trunk");
            if (!leavesMaterial) leavesMaterial = Resources.Load<Material>("Materials/Leaves");
            if(!_leavesMesh) _leavesMesh = Resources.GetBuiltinResource<Mesh>("Sphere.fbx");
            
            
            if(!Directory.Exists(Application.persistentDataPath+"/TreeMeshData"))
                Directory.CreateDirectory(Application.persistentDataPath+"/TreeMeshData");
            if (Directory.GetFiles(Application.persistentDataPath+"/TreeMeshData").Length != heightMapSettings.numberOfTreePrefabs)
            {
                for (int i = 0; i < heightMapSettings.numberOfTreePrefabs; i++)
                {
                    GameObject tree = gen(i, heightMapSettings.noiseSettings.seed);
                    BinaryFormatter binaryFormatter = new BinaryFormatter();
                    TreeMeshData treeMeshData = new TreeMeshData(tree.GetComponent<MeshFilter>().sharedMesh,tree.GetComponent<Renderer>());
                    using (FileStream fileStream = File.Open(Application.persistentDataPath+"/TreeMeshData/tree_"+i+".mesh", FileMode.Create))
                    {
                        binaryFormatter.Serialize(fileStream, treeMeshData);
                    }
                }
            }
        }
    ...
    public GameObject gen(int i,int seed)
    {
        ...
        _trunkThickness = Random.Range(0.60f, 0.75f);
        _floorHeight = Random.Range(0.25f, 0.3f);
        _firstBranchHeight = Random.Range(0.15f, 0.3f);
    ...
    {
        return (new Vector3(Random.Range(-coneAngularAmplitude, coneAngularAmplitude),
            Random.Range(-coneAngularAmplitude, coneAngularAmplitude),
            Random.Range(-coneAngularAmplitude, coneAngularAmplitude)) / 100f + direction).normalized;
    }
    ...
    Mesh GenBranch(GameObject tree, int basePolygon, Vector3[] startVertices, Vector2[] startUv, float thickness,
        float floorHeight, int floorNumber, Vector3 startingPos, Vector3 startingDirection, float angularOffset,
        float distorsionCone, float roughness, float branchDensity, int recursionLevel)
    {
        ...
        for (int i = 1; i < vertices.Length / basePolygon; ++i)
        {
            ...
            for (int j = 0; j < basePolygon; ++j)
            {
                ...
                Vector3 pos = new Vector3(Mathf.Cos(j * angularStep + angularOffset), 0f,
                    Mathf.Sin(j * angularStep + angularOffset)) + randomness;
            ...
            if (Happens(branchDensity) && recursionLevel > 0 && i >= floorNumber * _firstBranchHeight &&
                basePolygon >= 4 && i < vertices.Length / basePolygon - 1) // split!
            {
                int subBasePolygon = Random.Range(Mathf.Max(2, basePolygon / 3),
                    Mathf.Min(2 * basePolygon / 3, basePolygon - 1)) + 1;
                ...
                float newAngularOffset = angularOffset +
                                         Vector3.SignedAngle(subStartVertices[0] - mid,
                                             vertices[i * basePolygon] - pivot, growDirection) * Mathf.Deg2Rad;
                ...
                float newAngularOffset2 = angularOffset +
                                          Vector3.SignedAngle(subStartVertices2[0] - mid2,
                                              vertices[i * basePolygon] - pivot, growDirection) * Mathf.Deg2Rad;
                ...
                Mesh branch1 = GenBranch(tree, subBasePolygon, subStartVertices, subStartUv,
                    ...
                    floorNumber - i, subStartingPos,
                    getRandomVectorInCone(distorsionCone,
                        (growDirection + distorsionCone / 45f * (mid - pivot).normalized).normalized),
                ...
                Mesh branch2 = GenBranch(tree, subBasePolygon2, subStartVertices2, subStartUv2,
                    ...
                    floorNumber - i, subStartingPos2,
                    getRandomVectorInCone(distorsionCone,
                        (growDirection + distorsionCone / 45f * (mid2 - pivot).normalized).normalized),
            ...
            currentThickness =
                Mathf.Pow(Mathf.Asin((floorNumber - i) / (float)(floorNumber) * 2f - 1f) / Mathf.PI + 0.5f,
                    _reductionRate) * thickness;
